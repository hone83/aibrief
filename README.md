...
전략은 "네트워크 먼저, 실패하면 캐시"다.

다만 그냥 `fetch`를 하면 **브라우저의 HTTP 캐시가 먼저 답해버려서** 네트워크까지 가지도
않는다(깃허브 Pages가 HTML에 10분짜리 캐시를 건다). 그래서 HTML과 색인 JSON만
`cache: 'reload'`로 요청해 HTTP 캐시를 건너뛴다.

그래도 홈 화면 앱은 한 번 연 화면을 그대로 들고 있는 성질이 있어서, 페이지가 열릴 때와
앱을 다시 앞으로 가져올 때 `dates.json`을 캐시 없이 확인한다. 지금 보는 것보다 새 브리핑이
있으면 아래에 띠가 뜨고, **열기**를 누르면 모든 캐시를 지우고 새로 받는다.
